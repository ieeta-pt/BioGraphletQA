...
readonly PER_DEVICE_EVAL_BATCH_SIZE=16
...
readonly WARMUP_STEPS=100
readonly MAX_SEQ_LENGTH=512

# --- Base Directories ---
readonly DATA_DIR="../data/seqcls/$TASK_NAME"
...
train_and_evaluate() {
    ...
    local epoch=$6 # Now accepting seed as a parameter
    ...
    python3 -u seqcls/run_seqcls.py \
...
for seed in "${SEEDS[@]}"; do
    ...
    DATA=synth_20000
    initial_outdir="$SEED_RUNS_DIR/$MODEL_NAME-$DATA-e3"
    train_and_evaluate "$INITIAL_MODEL_PATH" "$DATA_DIR/$DATA.json" "$initial_outdir" "epoch" "$seed" 3
    ...
    # 100 200
    for train_subset in  300; do
        subset_outdir="$SEED_RUNS_DIR/$MODEL_NAME-$DATA-train_${train_subset}-e30"
        train_and_evaluate "$FINE_TUNED_MODEL_PATH" "$DATA_DIR/train_${train_subset}.json" "$subset_outdir" "no" "$seed" 30
    done

    # 3. Final run on the full training data
    echo "--- Starting Final Run on Full Training Data for Seed: $seed ---"
    final_outdir="$SEED_RUNS_DIR/$MODEL_NAME-$DATA-train_full-e30"
    train_and_evaluate "$FINE_TUNED_MODEL_PATH" "$DATA_DIR/train.json" "$final_outdir" "no" "$seed" 30
